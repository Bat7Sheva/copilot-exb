...
export const DataUrlInput = (props: UrlInputProps) => {
  ...
  return <div className='data-url-input w-100 h-100 p-4' css={style}>
    ...
    {/* geonet end */}
      ...
      <Button onClick={onAdd} type='primary' disabled={!urlResult.value || !urlResult.valid} className='px-4 w-100 add-btn' title={translate('add')} aria-label={translate('add')}>  { /*add-btn added class 'add-btn' */}
        {translate('add')}
      </Button>
    </div>
...
const style = css`
  ...
  .url-input-label {
    ...
    margin-bottom: 8px;
    // color: var(--dark-800);
    color: var(--secondary-100); //geonet
  }
  ...
  .add-btn {
    background: rgb(62, 187, 168) !important;
    border-radius: 8px;
  ...
  button.jimu-btn.text-left.jimu-dropdown-button.dropdown-button.app-root-emotion-cache-rtl-157vj9i.btn.btn-default.btn-sm, .dropdown-menu.inner-popper.jimu-dropdown-menu.popper.show {
    border: 1px solid var(--secondary-900) !important;
    background-color: var(--primary) !important;
    color: white;
    // width: 100%;

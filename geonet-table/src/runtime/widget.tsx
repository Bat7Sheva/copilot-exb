...
> {
  ...
  initializeCurrentMapWidget() {
    ...
    if (currentContainerMap?.length > 0) {
      this.mapWidgetId = currentContainerMap[0];
    }
  }
  ...
  onDataSourceInfoChange = (
    ...
    if (outputReapply || tabChange || dsCreated || sourceVersionChange || clientVersionChange) {
      if (dataSource?.dataViewId !== SELECTION_DATA_VIEW_ID && curLayer?.dataActionObject) return
    ...
    if (dataSourceUsed) {
      ...
      const tablePromise = cancelablePromise.cancelable(this.getFeatureLayer(dataSourceUsed, dataRecords).then(async layer => {
        if (newId !== this.currentRequestId || !layer || !this.FeatureTable || !this.refs.currentEl || !this.refs.tableContainer || !document) {
          // if (newId !== this.currentRequestId || !layer || !this.FeatureTable || !this.refs.currentEl || !document) {
        ...
        const isNormalPermission = isHonorWebmap ? (accessible || privilegeEditable) : curLayerConfig.enableEdit && (accessible || privilegeEditable)
        let editable = false
        ...
        if (isHonorWebmap) { //  && dataSource.isDataView && dataSource?.dataViewId !== OUTPUT_DATA_VIEW_ID
          ...
          tableTemplate = popupInfo?.fieldInfos
        ...
        if (curLayerConfig.enableSelect) {
          const rowClickFn = ({ context, native }) => {
            ...
            this.timerFn = setTimeout(() => {
              ...
              if (isViewSnapshot) {
                this.dsAsyncSelectTable(layerDs, selectedIds, true)
              } else {
  ...
  getColumnElementFromHeaderCell = (thElement: HTMLElement): HTMLElement | null => {
    const slot = thElement.querySelector("slot");
    if (!slot) return null;
    ...
    return <div className='top-button-list'>
      ...
      <div className='top-button ml-2'>
        <Button
          ...
          icon
          title={this.formatMessage('clearFilter')}
